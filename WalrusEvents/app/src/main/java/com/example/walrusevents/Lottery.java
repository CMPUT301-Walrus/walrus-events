...
public class Lottery {
    ...
    public boolean drawToCapacity(List<Entry> entrants, int capacity) {
        ...
        if(limit >= pending.size()) {
            for(Entry entrant : pending) {
                if(invite(entrant) < 0) return false; /* Draw failed: non-PENDING entrant found in the draw */
            }
            return true;
        }
        ...
        for(int i = 0; i < limit; i++) {
            ...
            if(invite(pending.get(index)) < 0) return false; /* Draw failed: attempted to invite non-PENDING entrant */
    ...
    /**
     * This method allows the caller to specify a specific number of entrants to draw for seating. This will still be limited by the event capacity.
     * @param entrants
     *  This is the list of entrants. It consists of a user id that can be used to refer back to their profile and the current status of their application
     * @param capacity
     *  This is the total number of entrants that can be enrolled in the event. capacity >= (number of INVITED + number of ACCEPTED)
     * @param x
     *  The specified amount of entrants to draw
     * @return
     *  Returns -1 if the draw fails
     */
    public int drawX(List<Entry> entrants, int capacity, int x) {
        return 0;
    }
